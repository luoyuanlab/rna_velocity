...
To systematically evaluate current models, we benchmarked them on two well-characterized datasets, mouse gastrulation erythroid lineage and mouse dentate gyrus, both of which are widely used due to their rich biological annotation and challenging dynamic structures. The dentate gyrus neurogenesis single cell data can be extracted using scVelo command scvelo.datasets.dentategyrus(). The mouse gastrulation subset to erythroid lineage data can be extracted via scVelo command scvelo.datasets.gastrulation_erythroid().
